...
def _assessment_age(assessment: dict) -> tuple[float, int]:
    ...
    return float(age), maximum_age


def _verified_control(assessment: dict, account_id: str) -> dict:
    ...
    if (
        ...
        or (control["event_count"] == 0 and control["latest_event_sha256"] is not None)
        or (
            control["event_count"] > 0
            and _sha256(control["latest_event_sha256"], "startup control event identity")
            is None
        )
        or canonical_sha256(control) != assessment["operational_control_sha256"]
    ):
        raise ValueError("startup operational control is invalid")
    return control


def _verified_recovery_state(assessment: dict) -> tuple[list[str], list[str], list[str], list[str]]:
    ...
    for item in resolved:
        ...
        require_identifier(item["idempotency_key"], "resolved submission identifier")
        require_identifier(item["resolution_key"], "submission resolution identifier")
        _sha256(item["resolution_sha256"], "submission resolution identity")
        if item["outcome"] not in _RESOLUTION_OUTCOMES or item["retry_permitted"] is not False:
            raise ValueError("resolved startup submission is invalid")
    ...
    return uncertain, resolved_open, uncertain_cancellations, incomplete_stops
...
def verify_assessment(
    ...
    *,
    ...
    age, maximum_age = _assessment_age(assessment)
    ...
    control = _verified_control(assessment, account_id)
    uncertain, resolved_open, uncertain_cancellations, incomplete_stops = (
        _verified_recovery_state(assessment)
    )
    expected_reasons = _blocker_reasons(
        account_active=assessment["account_active"],
        reconciliation_status=assessment["reconciliation_status"],
        reconciliation_age_seconds=age,
        max_reconciliation_age_seconds=maximum_age,
        uncertain_submissions=tuple(uncertain),
        resolved_open_submissions=tuple(resolved_open),
        uncertain_cancellations=tuple(uncertain_cancellations),
        incomplete_emergency_stops=tuple(incomplete_stops),
