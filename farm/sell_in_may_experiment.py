#!/usr/bin/env python
"""Run and evaluate charter SELL-IN-MAY-SPY-2026-09-07-v1.

Exactly four isolated walk-forward replays are permitted: one seasonal candidate
and one exposure-matched static control under baseline and doubled costs. Nothing
here registers a paper portfolio or opens a parameter grid.
"""
...
CHARTER_ID = "SELL-IN-MAY-SPY-2026-09-07-v1"
COMPARISON_PROTOCOL = "sell-in-may-spy-2026-09-07-v1"
CANDIDATE_ID = "sell_in_may_spy"
CONTROL_ID = "sell_in_may_static_4902"
ANCHOR = date(2026, 9, 4)
INITIAL_CASH = 39_000.0
PROFILES = (execution.BASELINE.id, execution.COST_2X.id)
OUT_DIR = DATA_DIR / "reports" / "experiments" / "sell-in-may-spy-v1"
SCRATCH_ROOT = REPO_ROOT / "scratch" / "sell-in-may-spy-v1"
DATA_CLASS = "fixed_etf_history"
RESEARCH_STRATEGIES = {
    CONTROL_ID: SellInMayStaticExposure,
    CANDIDATE_ID: SellInMaySpy,
}
...
def _config(config_id: str, profile_id: str) -> dict:
    ...
    return {
        "id": config_id,
        "name": ("Sell-in-May SPY" if candidate
                 else "Static 49.0229% SPY / 50.9771% BIL"),
        "strategy": config_id,
        "cadence": "daily",
        "params": {
            "assets": list(ASSETS),
            "risk_on_months": [11, 12, 1, 2, 3, 4],
            "static_spy_weight": STATIC_SPY_WEIGHT,
        },
        "charter_id": CHARTER_ID,
        "profile_id": profile_id,
        "description": (
            "Hold SPY in November-April and BIL in May-October."
            if candidate else
            "Hold a fixed exposure-matched SPY/BIL mix on seasonal transition dates."
        ),
        "expectation": "Positive net seasonal-timing excess over static exposure.",
...
def capture_price_input(live_con, _scratch_con, start, end, sessions) -> dict:
    """Fingerprint exact SPY/BIL bars and verify the frozen seasonal exposure."""
    ...
    winter_count = sum(is_winter_session(d) for d in sessions)
    return {
        "class": DATA_CLASS,
        "assets": list(ASSETS),
        "session_count": len(sessions),
        "winter_session_count": winter_count,
        "derived_spy_weight": winter_count / len(sessions) if sessions else None,
...
def run_replays(live_con, *, out_dir: Path = OUT_DIR,
    ...
    if collisions:
        raise RuntimeError(f"sell-in-May research strategies in registry: {collisions}")
...
def _input_complete(result: dict) -> bool:
    ...
    return bool(
        ...
        and value.get("winter_session_count") == 1480
        and math.isclose(
...
def render(report: dict) -> str:
    ...
    return paired.render_experiment(
        report,
        charter_id=CHARTER_ID,
        title="Sell-in-May SPY — historical charter result",
        introduction=(
            "One frozen November-April rule is compared with a static SPY/BIL control "
            "matched to its predeclared calendar exposure."
        ),
        data_boundary=(
            f"The input contains {source.get('session_count', 0):,} complete SPY/BIL "
            f"sessions from {source.get('start')} through {source.get('end')}; "
            f"{source.get('winter_session_count', 0):,} are in November-April. Exact "
            f"bar fingerprint: `{source.get('sha256')}`."
        ),
        closing=(
...
def main() -> int:
    ...
    print(f"[sell-in-may] wrote {md_path} and {json_path}")
    return 0
