"""The release-review migration is backup-gated, scoped, and rollback-safe."""
...
from tools import backup_database, migrate_agent_release_review
...
def _setup(tmp_path: Path, monkeypatch) -> tuple[Path, Path, dict]:
    ...
    monkeypatch.setattr(
        migrate_agent_release_review.backup_database,
        "verify_backup",
...
def test_migration_creates_only_empty_release_review_table(tmp_path, monkeypatch):
    root, database, verified = _setup(tmp_path, monkeypatch)
    before = _snapshot(database)

    result = migrate_agent_release_review.migrate(
        database,
        tmp_path / "backup",
    ...
    assert result["status"] == "release_review_schema_installed"
    assert result["created_table"] == agent_release_review_store.TABLE
    assert result["retained_table"] is None
    assert result["release_review_observation_count"] == 0
...
def test_migration_is_idempotent_with_fresh_matching_backup(tmp_path, monkeypatch):
    ...
    assert result["retained_table"] == agent_release_review_store.TABLE
...
def test_migration_rejects_wrong_or_stale_backup(
    ...
    with pytest.raises(
        migrate_agent_release_review.ReleaseReviewMigrationError,
        match=message,
    ):
        migrate_agent_release_review.migrate(
            database,
    ...
    try:
        ...
        con.close()
    assert agent_release_review_store.TABLE not in tables
...
def test_migration_rejects_incompatible_existing_schema(tmp_path, monkeypatch):
    ...
    con.execute(
        f"CREATE TABLE {agent_release_review_store.TABLE} "
    ...
    with pytest.raises(
        migrate_agent_release_review.ReleaseReviewMigrationError,
        match="existing release review schema is incompatible",
    ):
        migrate_agent_release_review.migrate(
            database,
            tmp_path / "backup",
...
def test_migration_rejects_nonempty_existing_ledger(tmp_path, monkeypatch):
    ...
    values = [1] + [
        f"value-{index}"
        for index in range(1, len(agent_release_review_store._COLUMNS))
    ]
    values[-1] = "f" * 64
    con.execute(
        f"INSERT INTO {agent_release_review_store.TABLE} VALUES "
        f"({', '.join('?' for _ in agent_release_review_store._COLUMNS)})",
        values,
    )
    con.close()
    current = _snapshot(database)
