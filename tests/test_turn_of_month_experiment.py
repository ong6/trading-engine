"""Turn-of-month experiment isolation and fail-closed decision gates."""
from copy import deepcopy

from farm import turn_of_month_experiment as experiment
...
def test_symmetric_terminal_pending_is_allowed():
    results = _results()
    for profile in experiment.PROFILES:
        for config_id in (experiment.CANDIDATE_ID, experiment.CONTROL_ID):
            fold = results[profile][config_id]["folds"][-1]
            fold["pending_orders"] = [{
                "id": 1, "ticker": "SPY", "side": "buy",
                "signal_date": fold["last_session"],
            }]
            fold["n_pending"] = 1
    assert experiment.evaluate(results)["gates"][
        "execution_data_and_accounting_clean"
    ] is True


def test_nonterminal_pending_rejects_v1():
    results = _results()
    for profile in experiment.PROFILES:
        for config_id in (experiment.CANDIDATE_ID, experiment.CONTROL_ID):
            fold = results[profile][config_id]["folds"][-1]
...
def test_replays_scope_registry_and_cleanup(monkeypatch, tmp_path):
    ...
    def fake_run_book(*args, **kwargs):
        assert REGISTRY[experiment.CONTROL_ID] is experiment.TurnOfMonthStaticExposure
        assert REGISTRY[experiment.CANDIDATE_ID] is experiment.TurnOfMonthSpy
