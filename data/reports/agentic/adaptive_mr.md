...
_role **tuner** (weekly mean-reversion params) · param version **v1** · 0 applied / 0 rejected change(s) · 1 lesson(s) · generated 2026-08-04 09:54 UTC._

Charter: `agents/adaptive_mr/charter.md` (frozen — bounds, objective and kill criterion are not re-registrable mid-stream).

> **No data yet — no `sim_equity` rows yet: adaptive_mr, adaptive_mr_frozen.** Nothing below is measurable until both this book and `adaptive_mr_frozen` have `sim_equity` rows on a shared session.
