...
_role **gater** (vetoes / downscales entries on news) · param version **v1** · 0 applied / 0 rejected change(s) · 1 lesson(s) · generated 2026-08-04 09:54 UTC._

Charter: `agents/news_gated_momo/charter.md` (frozen — bounds, objective and kill criterion are not re-registrable mid-stream).

> **No data yet — no `sim_equity` rows yet: news_gated_momo.** Nothing below is measurable until both this book and `momo_stopped` have `sim_equity` rows on a shared session.
